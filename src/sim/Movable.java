...
public class Movable implements Element {
    ...
    public Movable(Segment position, double offset) {
        ...
        log("Sim: Movable placed on "+sector);
        SX.instance().setStatusBit(sector.address, sector.bitpos, 1);
    }
    ...
    public void update() {
        ...
        if (offset < 0) {
            ...
            if (position.start.isDeadEnd()) {
                offset = 0;
                log("Sim: "+getId()+" reached an end at "+position.start.sector);
            } else {
            ...
            if (position.stop.isDeadEnd()) {
                ...
                log("Sim: "+getId()+" reached an end at "+position.stop.sector);
            } else {
        ...
        if (position!=newPosition) {
            // log
            // Simulator.log(getId()+" left "+position+" for "+newPosition);
            // not needed anymore
        ...
        if (newSector!=oldSector) {
            log("Sim: "+getId()+" has left "+oldSector+" for "+newSector);
            SX.instance().setStatusBit(newSector.address, newSector.bitpos, 1);
            SX.instance().setStatusBit(oldSector.address, oldSector.bitpos, 0);
        }
    }
